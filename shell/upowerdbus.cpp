...
void UPowerDBus::DeviceChanged() {
    if (!isUpdatingDisplay) {
        ...
        if (QDBusConnection::sessionBus().interface()->registeredServiceNames().value().contains("org.kde.kdeconnect")) {
            ...
            QDBusPendingCallWatcher* watcher = new QDBusPendingCallWatcher(QDBusConnection::sessionBus().asyncCall(devicesMessage));
            ...
            QDBusReply<QStringList> connectedDevices = watcher->reply();
